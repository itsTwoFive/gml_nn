...
/// @brief Sigmoid of the x value
/// @param x Input value of the function
/// @return Value of the sigmoid(x)
float sigmoid(float x){
    return 1.0f/(1.0f+ expf(-x));
}

/// @brief Derivate of the sigmoid at x
/// @param x Input value of the function
/// @return Value of the derivate
float dsigmoid(float x){
    return sigmoid(x)*(1-sigmoid(x));
}

/// @brief Allocate dynamic memory for 1 layer and initialize weights values ramdomly
/// @param previous_n Number of inputs of the layer
/// @param current_n Number of outputs/neurons of the layer
/// @param weights Matrix containing all the weights
/// @param biases Matrix containing all the biases
...
/// @brief Saves in a csv an area map for the results of the network
void saveAreas2DFile(){
    float step = .01;
    
    FILE *fp = fopen("areas.csv", "w"); 
    ...
    for (float x = 0; x < 1; x +=step)
    {
        ...
        {
            float punto[] = {x,y};
            if (feedforwardNetwork(punto)>0.5) fprintf(fp, "%f, %f, %i\n", x, y,0);
        }
    }
    for (float x = 0; x < 1; x +=step)
    {
...
int main()
{
    // Initialize the Ws and Bs values to random
    initRandVariables();
    ...
    if(PRINT_COST){
        printed_train(num_it);
         ...
         //  Train for those epochs
    printf("S. Coste %f.\n",cost());
    for (size_t i = 0; i < num_it; i++)
    {
        trainNetwork();
        if (i%1 == 0) printf("%zu. Coste %f.\n",i,cost());
