...
int get_number_lines_from_file(FILE* fp){
    char c;
    int num_filas = 0;
...
int get_number_cases(char filename[]){
    ...
    int numlines = get_number_lines_from_file(fp);
    fclose(fp);
    return numlines;
}
...
parser_result parse_data(char filename[],int num_inputs){
    ...
    int num_lines = get_number_lines_from_file(fp);
    fseek(fp,0,SEEK_SET);
...
void change_all_values_for(double ** data,int size_x, int size_y,double actual, double new){
    for (int i = 0; i < size_y; i++)
    {
        ...
        {
            if(fabs(data[i][j]-actual) < EPSILON_DISTANCE){
                data[i][j] = new;
            }
        }
    }
...
double **from_integer_to_binary_classes(double** list,int num_case,int *num_classes){
    ...
    *num_classes = max +1;
    ...
    double ** res_data = array_alloc(num_case,*num_classes);
    for (int i = 0; i < num_case; i++)
    {
        int num = (int) list[i][0];
        res_data[i][num] = 1.0;
