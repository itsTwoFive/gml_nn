...
void rewind_print_close(FILE * tmp){
    rewind(tmp);
    char buffer[256];
...
int choose_best_class(double *outputs,int num_out){
    double max_dist = -5;
    int current_best = 0;
    for (int i = 0; i < num_out; i++)
    {
        if (outputs[i] > max_dist){
            max_dist = outputs[i]; 
            current_best = i;
        }
    }
...
void plot_2d_data_for_binary(double** data_in,double** data_out, int num_cases,int num_out,double* ranges){
    ...
    fprintf(pipe, "plot '-' w points lw 1.5");
    for (int i = 0; i < num_out-1; i++)
    {
        fprintf(pipe, ", '-' w points lw 1");
    }
    fprintf(pipe, "\n");
...
double* find_minmax(double **data,int param ,int size) {
...
void minmax_normalization(double** data,int param,int size){
    double* minmax = find_minmax(data,param,size);
    double min = minmax[0];
    double max = minmax[1];

    for (int i = 0; i < size; i++) {
        data[i][param] = 2*(data[i][param] - min) / (max - min)-1;
        if(isnan(data[i][param])){
            data[i][param] = -1;
        }
    }
}